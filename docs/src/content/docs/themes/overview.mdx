...
├── design.md              # Optional shared colors and fonts
├── src/
...
## Defining shared design tokens

Create `design.md` in the project root when the theme has shared colors and fonts. EmDash reads this file during the Astro build and includes the recognized values in `SiteSettings.theme`. The public theme, widgets, email templates, and AI-generated public components can then use the same values.

Use `## Colors` and `## Fonts` headings with `Name: value` entries. Color values must be six-digit hexadecimal colors. Font values must be a font stack containing letters, numbers, spaces, commas, periods, apostrophes, quotes, or hyphens.

The following file defines a complete design token set:
...
EmDash accepts the Spanish section names `Colores` and `Fuentes` too. Unrecognized entries are ignored. If `design.md` is absent, the compiler leaves the design settings unset. Values declared explicitly in the seed file take precedence over values extracted from `design.md`.

You can review or change the primary and secondary colors, including their hover and foreground variants, in `Admin → Settings → General → Design`. Structural and semantic tokens such as background, surface, text, muted text, borders, fonts, links, and status colors remain managed by the design file so widgets and emails keep the intended visual harmony. Changes made there are stored in the site settings database and are available to the public site at runtime.
