...
Site settings are global configuration values for your site: title, tagline, logo, social links, display preferences, and shared design tokens. Administrators manage these through the admin interface, and you access them in your templates.
...
`settings.theme` contains the colors and font stacks shared by the public theme, widgets, email templates, and AI-generated public components. The standard public theme styles map these values to CSS variables such as `--color-accent`, `--color-bg`, `--color-surface`, `--color-text`, `--color-border`, `--font-body`, and `--font-heading`.

Set these values in `Admin → Settings → General → Design`, or define them in a root-level `design.md` file for build-time extraction. See [Defining shared design tokens](/themes/overview/#defining-shared-design-tokens) for the Markdown format.
